 
import { Component , SyntheticEvent } from "react";
import { Button, Col, Container, Row } from "react-bootstrap";
import { Unsubscribe } from "redux";

import VacationsModel from "../../../Models/vacationModel";
import store from "../../../Redux/Store";
import { downloadedVacations } from "../../../Redux/vacationsState";
import globals from "../../../Service/globals";
import jwtAxios from "../../../Service/JwtService";
import {History} from "history"

import AddVacation from "../AddVacation/AddVacation";
import EditVacation from "../EditVacation/EditVacation";
import VacationCard from "../../LayoutArea/vacationCard/vacationCard";
import "./AdminHomeCC.css";
import notify from "../../../Service/Notify";
import { NavLink } from "react-router-dom";

 
interface AdminHomeCCProps {
    history: History ;
  
};


interface AdminHomeCCState {
	vacations: VacationsModel[]  ;
}




class AdminHomeCC extends Component< AdminHomeCCProps, AdminHomeCCState> {

    private unsubscribeStore : Unsubscribe ;

    public constructor(props: AdminHomeCCProps) {
        super(props);
  
        this.state = {
			vacations: store.getState().vacationState.vacation 
        };
    }

public async componentDidMount(){
try{
    if(!store.getState().userState.user){
      this.props.history.push("/auth/sign-in") ;
        return
     };
   this.unsubscribeStore  = store.subscribe(()=>{
        this.setState({ vacations: store.getState().vacationState.vacation });
    });

    if(this.state.vacations.length === 0) {
   const response = await jwtAxios.get<VacationsModel[]>(globals.urlAdmin+"/") ;
   store.dispatch(downloadedVacations(response.data)) ;
    }
}
catch(err){
    notify.error(err)
}};


public componentWillUnmount(){
    this.unsubscribeStore();
};


private deleteVacation = async (args:SyntheticEvent) => {
        try{

        const answer = window.confirm("Are you sure you want to delete this vacation ?")
        if(!answer) return
       const vacationId = (args.target as HTMLButtonElement).id
       await jwtAxios.post(globals.urlAdmin+"/delete", {vacationId :vacationId} );
       notify.success("Successfully delete");

        }
        catch(err){
            notify.error(err);
        }
    }


public render(): JSX.Element {
        return (
    <div>
            <br />

<svg xmlns='http://www.w3.org/2000/svg' style={{ display: "none" }}>
<symbol id='geo-fill' viewBox='0 0 16 16'>
    <path
        fillRule='evenodd'
        d='M4 4a4 4 0 1 1 4.5 3.969V13.5a.5.5 0 0 1-1 0V7.97A4 4 0 0 1 4 3.999zm2.493 8.574a.5.5 0 0 1-.411.575c-.712.118-1.28.295-1.655.493a1.319 1.319 0 0 0-.37.265.301.301 0 0 0-.057.09V14l.002.008a.147.147 0 0 0 .016.033.617.617 0 0 0 .145.15c.165.13.435.27.813.395.751.25 1.82.414 3.024.414s2.273-.163 3.024-.414c.378-.126.648-.265.813-.395a.619.619 0 0 0 .146-.15.148.148 0 0 0 .015-.033L12 14v-.004a.301.301 0 0 0-.057-.09 1.318 1.318 0 0 0-.37-.264c-.376-.198-.943-.375-1.655-.493a.5.5 0 1 1 .164-.986c.77.127 1.452.328 1.957.594C12.5 13 13 13.4 13 14c0 .426-.26.752-.544.977-.29.228-.68.413-1.116.558-.878.293-2.059.465-3.34.465-1.281 0-2.462-.172-3.34-.465-.436-.145-.826-.33-1.116-.558C3.26 14.752 3 14.426 3 14c0-.599.5-1 .961-1.243.505-.266 1.187-.467 1.957-.594a.5.5 0 0 1 .575.411z'
    />
</symbol>
</svg>
<h1 className='visually-hidden'>Management Dashboard</h1>
<br />
<br />

<h2 className='pb-2 border-bottom'>Active Lists</h2>
<div className="innerMenu"  ><AddVacation /><span> </span><EditVacation /> <span> </span> <NavLink to="/admin-home/reports" className="btn btn-dark" >Reports</NavLink></div> 
<br />

<Container>
<Row className="AdminHomeCC">

{this.state.vacations.map((v, index) => {

  
return <Col key={index} className="outerCard m-3"  >
    <Col className="innerCard"> 
    <Row className='row gx-5'>
    <Col className='col' >
        <div
            className='feature-icon bg-primary bg-gradient'>
            <svg className='bi' width='1em' height='1em'>
                <use xlinkHref='#geo-fill' />
            </svg>
        </div>
    </Col>
    </Row>  
    <VacationCard vacation={v} />
    <Row>
        <Col><Button id={v.vacationId.toLocaleString()} variant="danger" onClick={this.deleteVacation} >Delete</Button></Col>
    </Row>
    </Col>
</Col>
    
     
})}
   </Row>
</Container>
<div className='b-example-divider'></div>
    </div>
        );
    }
}

export default AdminHomeCC;

